...
states = {
        ...
        'Choco': {'capital': 'Quibdo', 'url': 'Choco,+Colombia', 'abrv': 'Choco', 'id': 5181, 'lat': 7.151754, 'lon': -76.732422, 'type': 'region', 'reg_type': 'province'},
        ...
        'La Guajira': {'capital': 'Riohacha', 'url': 'Guajira,+Colombia', 'abrv': 'Guajira', 'id': 5108, 'lat': 11.3799, 'lon': -73.209346, 'type': 'region', 'reg_type': 'province'},
...
def getNextPageURL(d):
    ...
    linesPerPage = int(fnmatch.filter(page[len(page) - 1].split('&'), 'rows*')[0].split('=')[1])
...
def getHotelServices(d, hotelsDF):
    ...
    hotelServices = []
    for i in range(len(hotelsDF)):
        d.get(hotelsDF['hotelUrl'][i])
        services = [item.text for item in d.find_elements_by_css_selector(tags['hotelServices']['tag'])]      
...
def getHotelReview(block, hotel):
    ...
    for user in block:
        ...
        if index1 != -1 or index2 != -1:
            index3 = comment.index(fnmatch.filter(comment, tags['stayedTitle']['tag'] + '*')[0])
...
def getHotelReviews(d, hotel):
    ...
    if hotel['reviewNumber'] > rowsPerPage:
        while True:
            try:
                nextURLBlock = d.find_elements_by_xpath(tags['nextReviewURL']['tag'])
                lastURL = [elem.get_attribute('href') for elem in nextURLBlock][-1]
                url = '&'.join([i for i in lastURL.split('&') if i.find('offset') == -1])
                break
            except:
                continue
...
def getHotelComments(d, hotelsDF):
    ...
    hotel_scores = []
    hotel_reviews = []
    for i in range(len(hotel_reviews), len(hotelsDF)):
        print(hotelsDF['hotelName'][i] + '...')
        if hotelsDF['reviewNumber'][i] != 0:
            urlHotelReviews = hotelsDF['hotelUrl'][i][:-9]\
                + tags['tabReview']['tag']
            d.get(urlHotelReviews)

            hotel_scores.append(getHotelScoreBars(d))
            hotel_reviews.append(getHotelReviews(d, hotelsDF.iloc[i]))
        else:
            hotel_scores.append({})
...
geckoDriverDir = rootDir + 'geckodriver/geckodriver'
d = webdriver.Firefox()

state = states['Amazonas']
for state in states:
    hotelsDF = getHotelData(d, state, '')

    urlNextPage, hotelsPerPage, totalPages, hotelsNum = getNextPageURL(d)
    startInstance = hotelsPerPage
    hotelCounter = hotelsPerPage + 1
    while (hotelCounter <= hotelsNum):
        newURL = urlNextPage + tags['offset']['tag'] + str(startInstance)
        hotelsDF = hotelsDF.append(getHotelData(d, state, newURL), ignore_index = True)
        startInstance += hotelsPerPage
        hotelCounter += hotelsPerPage
    
    hotelsDF, services = getHotelServices(d, hotelsDF)
    hotelsDF = getHotelAddress(d, hotelsDF)

    #hotelsDF = pd.read_csv(datasetDir + state['abrv'] + '_hotels.csv')
    hotelsDF, reviews, scores = getHotelComments(d, hotelsDF) # Get hotel comments

    servicesDF = pd.DataFrame(services)
    servicesDF['hotelID'] = hotelsDF['hotelID']
    servicesDF.to_csv(datasetDir + state['abrv'] + '_services.csv',index = False, header=True)
    
    scoresDF = pd.DataFrame(scores)
    scoresDF['hotelID'] = hotelsDF['hotelID']
    scoresDF.to_csv(datasetDir + state['abrv'] + '_scores.csv',index = False, header=True)    
    
    reviewsDF = pd.DataFrame(reviews)
    reviewsDF['hotelID'] = hotelsDF['hotelID']
    reviewsDF.to_csv(datasetDir + state['abrv'] + '_reviews.csv',index = False, header=True)

    hotelsDF.to_csv(datasetDir + state['abrv'] + '_hotels.csv',index = False, header=True)
 
 
    ## Get the hotel activities dataframe
    # #servicesDF = pd.read_csv('/home/luz/Python/web scraping/services.csv')
    # activityType = []
    # activityType = pd.DataFrame(servicesDF['Actividades'].explode().unique(), columns = ['activities'])
    # activityType.to_csv('/home/luz/Python/web scraping/activityType.csv',index = False, header=True)
    # ## Get the hotel wellness dataframes
    # wellnessType = []
    # wellnessType = pd.DataFrame(servicesDF['Instalaciones de wellness'].explode().unique(), columns = ['wellness'])
    # wellnessType.to_csv('/home/luz/Python/web scraping/wellnessType.csv',index = False, header=True)
    
    # ## Create the turist experience column in the dataframe
    # touristExperience = pd.read_csv('/home/luz/Python/web scraping/hotelExperiences.csv')
      
    # touristExp = []
    # for j in range(len(servicesDF)):
    #     temp = []
    #     if str(servicesDF['Actividades'][j]) != 'nan': 
    #         item = servicesDF['Actividades'][j].replace('[','').replace(']','').replace(" '",'').replace("'",'').split(',')
    #         for i in item:
    #             temp.append(touristExperience[touristExperience['services'] == i]['experience'].tolist()[0])
    #         touristExp.append(list(set(temp)))
    #     else:
    #         touristExp.append([])    

    # servicesDF['activityExperience'] = touristExp   
    
    # touristExp = []
    # for j in range(len(servicesDF)):
    #     temp = []
    #     if str(servicesDF['Instalaciones de wellness'][j]) != 'nan': 
    #         item = servicesDF['Instalaciones de wellness'][j].replace('[','').replace(']','').replace(" '",'').replace("'",'').split(',')
    #         for i in item:
    #             if len(touristExperience[touristExperience['services'] == i]['experience'].tolist()) > 0:
    #                 temp.append(touristExperience[touristExperience['services'] == i]['experience'].tolist()[0])
    #         touristExp.append(list(set(temp)))
    #     else:
    #         touristExp.append([])    

    # servicesDF['wellnessExperience'] = touristExp   
    
    # servicesDF['touristExperience'] = servicesDF.apply(
